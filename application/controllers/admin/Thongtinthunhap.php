...
class Thongtinthunhap extends Admin {

	public function get_income($iduser)
	{
      ...
      $config['total_rows'] = $this->get_count_income_info($iduser);
       $config['per_page'] = $this->config->item('per_page');
      $config['base_url'] = home_url() .'/income-info';
      $this->pagination->initialize($config);
      $data['pagination'] = $this->pagination->create_links();
     	$start_row = intval($this->input->get('bat-dau'));
		$data['income_info'] = $this->get_income_info($iduser,$start_row, $config['per_page']);
